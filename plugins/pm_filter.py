...
async def filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        ...
        if len(btn) > 10: 
            ...
            if poster:
                await message.reply_photo(photo=poster, caption=f"<b>🎬𝐌𝐨𝐯𝐢𝐞/𝐒𝐞𝐫𝐢𝐞𝐬:{search}\n\n𝐅𝐑𝐎𝐌:◻⬜<a href="https://t.me/AJmovieLINKS">♛𝗧𝗛𝗘 𝗦𝗘𝗥𝗩𝗘𝗥♕</a>⬛◼  ‌‎ ­  ­  ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))
...
async def group(client, message):
    ...
    if 2 < len(message.text) < 50:    
        ...
        buttons.append(
            [InlineKeyboardButton(text=f"📃 𝗣𝗔𝗚𝗘 1/{data['total']}",callback_data="pages")]
        )
        poster=None
        if API_KEY:
            poster=await get_poster(search)
